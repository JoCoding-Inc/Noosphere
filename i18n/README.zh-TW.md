...
| GET | `/health` | 健康檢查 |
| POST | `/simulate` | 啟動新模擬 |
| GET | `/simulate-stream/{sim_id}` | 即時進度 SSE 串流 |
| GET | `/results/{sim_id}` | 取得完成的結果 |
| GET | `/history` | 列出所有模擬 |
| GET | `/export/{sim_id}` | 下載 PDF 報告 |
| POST | `/simulate/{sim_id}/cancel` | 取消執行中的模擬 |
| POST | `/simulate/{sim_id}/resume` | 恢復暫停的模擬 |
| DELETE | `/simulate/{sim_id}` | 刪除模擬 |
| GET | `/simulate/{sim_id}/status` | 查看模擬狀態 |
